...
export const AGENT_TOOLS: AgentTool[] = [
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: {
        prompt: { type: "string", description: "A clear visual description of the desired clip, e.g. 'a commercial airplane flying through white clouds, blue sky, seen from below'. Must match what you already showed the user in the plan. No separate negative-prompt field — fold negatives into the text (e.g. 'no text overlays, no watermark')." },
        duration_seconds: { type: "number", description: "Target only. One of 4, 6, 8, or 10 — the closest allowed value is used. Default 8." },
        aspect_ratio: { type: "string", enum: ["9:16", "16:9"], description: "Default '9:16' (match the project's vertical default unless the user's project is landscape)." },
        confirmed: {
          type: "boolean",
